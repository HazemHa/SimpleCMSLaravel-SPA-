    ...
    <v-toolbar flat color="white">
      ...
      <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ on }">
          <v-btn color="primary" dark class="mb-2" v-on="on">New Status</v-btn>
        </template>
        <v-card>
          <v-card-title>
...
if (confirm("Are you sure you want to delete this item?")) {
        this.$store
          .dispatch("status/deleteStatus", { id: item.id })
          .then((res) => {

            if (res.mutationStatus.id == -1) {
...
if (this.editedIndex > -1) {
        // update the record
        this.$store
          .dispatch("status/updateStatus", this.editedItem)
          .then((res) => {
            if (res.mutationStatus.id == -1) {
              console.log("cant update ");
            } else {
              Object.assign(this.data[this.editedIndex], res.mutationStatus);
            }
          })
          .catch(err => {
            console.log(err);
        ...
        this.$store          .dispatch("status/createStatus", this.editedItem)
          .then((res) => {
            this.data.push(res.mutationStatus);
          })
          .catch(err => {
            console.log(err);
          });
